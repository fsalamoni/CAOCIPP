 ...
 */
async function isPlatformAdmin(request) {
    var _a;
    if (!request.auth)
        return false;
 ...
 */
async function deleteCollectionForOrg(db, bulkWriter, collectionName, organizationId) {
    let deleted = 0;
 ...
 */
exports.deleteOrganization = (0, https_1.onCall)({ region: 'southamerica-east1', cors: true, timeoutSeconds: 540, memory: '512MiB' }, async (request) => {
    var _a, _b;
    if (!request.auth) {
    ...
    const orgName = String(((_a = orgSnap.data()) === null || _a === void 0 ? void 0 : _a.name) || '');
    ...
    if (!platformAdmin) {
        const membershipSnap = await db
            ...
            .get();
        const role = membershipSnap.exists ? (_b = membershipSnap.data()) === null || _b === void 0 ? void 0 : _b.role : null;
        isCreator = role === 'creator';
        if (!isCreator) {
            throw new https_1.HttpsError('permission-denied', 'Apenas o Criador da organização ou um administrador da plataforma podem excluí-la.');
        }
    }
    // 3. Confirmação pelo nome exato (guarda contra exclusão acidental).
    if (typeof confirmName !== 'string' || confirmName.trim() !== orgName.trim()) {
        throw new https_1.HttpsError('failed-precondition', 'A confirmação do nome da organização não coincide.');
    }
    console.log(`[DeleteOrg] Iniciando exclusão de ${organizationId} (${orgName}) por ${userId} ` +
    ...
    for (const collectionName of ORG_SCOPED_COLLECTIONS) {
        deletedByCollection[collectionName] = await deleteCollectionForOrg(db, bulkWriter, collectionName, organizationId);
    }
    ...
    console.log(`[DeleteOrg] Concluída exclusão de ${organizationId}. Removidos:`, JSON.stringify(deletedByCollection));
