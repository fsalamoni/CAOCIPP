...
exports.updateOrganization = (0, https_1.onCall)({ region: 'southamerica-east1' }, async (request) => {
    ...
    if (data.parceriaSettings !== undefined)
        updates.parceriaSettings = sanitizeParceriaSettings(data.parceriaSettings);
...
const PARCERIA_DEFAULT_CATEGORIAS = [];
// Deduplica + limita uma lista de strings. Retorna o default se o array
// resultante ficar vazio (apenas para listas obrigatórias).
function sanitizeStringList(input, max, defaultValue) {
    const arr = Array.isArray(input) ? input : [];
    const seen = new Set();
    const out = [];
    for (const item of arr) {
        const s = String(item !== null && item !== void 0 ? item : '').trim().slice(0, 100);
        if (!s || seen.has(s))
            continue;
        seen.add(s);
        out.push(s);
...
function sanitizeParceriaSettings(input) {
    return {
        tipos: sanitizeStringList(input === null || input === void 0 ? void 0 : input.tipos, 50, PARCERIA_DEFAULT_TIPOS),
        aditivoTipos: sanitizeStringList(input === null || input === void 0 ? void 0 : input.aditivoTipos, 30, PARCERIA_DEFAULT_ADITIVO_TIPOS),
        vigenciaOptions: sanitizeStringList(input === null || input === void 0 ? void 0 : input.vigenciaOptions, 30, PARCERIA_DEFAULT_VIGENCIA_OPTIONS),
        // categorias é opcional — pode ficar vazio sem cair no default.
        categorias: sanitizeStringList(input === null || input === void 0 ? void 0 : input.categorias, 30, PARCERIA_DEFAULT_CATEGORIAS),
        ...
        thirdPartyPhaseEnabled: (input === null || input === void 0 ? void 0 : input.thirdPartyPhaseEnabled) !== false,
        autoExtinguishOnEnd: (input === null || input === void 0 ? void 0 : input.autoExtinguishOnEnd) === true,
    };
}
// ============================================================================
