...
exports.runAnonymization = exports.previewAnonymization = void 0;
...
async function assertIsCreator(db, userId, organizationId) {
    var _a;
    ...
    if (!membershipSnap.exists || ((_a = membershipSnap.data()) === null || _a === void 0 ? void 0 : _a.role) !== 'creator') {
        throw new https_1.HttpsError('permission-denied', 'Apenas o Criador do órgão pode gerenciar a política de retenção.');
    }
}
function isEligible(data, cutoff) {
    if (data.anonymized === true)
        return false;
    if (!data.archived_date)
        return false;
    const archived = new Date(data.archived_date);
    if (isNaN(archived.getTime()))
        return false;
    return archived <= cutoff;
}
exports.previewAnonymization = (0, https_1.onCall)({ region: 'southamerica-east1' }, async (request) => {
    var _a;
    if (!request.auth) {
    ...
    const retentionConfig = (_a = orgSnap.data()) === null || _a === void 0 ? void 0 : _a.retentionConfig;
    const anonymizeAfterDays = Number(retentionConfig === null || retentionConfig === void 0 ? void 0 : retentionConfig.anonymizeAfterDays) || 365;
...
exports.runAnonymization = (0, https_1.onCall)({ region: 'southamerica-east1' }, async (request) => {
    var _a;
    if (!request.auth) {
        throw new https_1.HttpsError('unauthenticated', 'Authenticated user required');
    }
    ...
    if (ids.length > 300) {
        throw new https_1.HttpsError('invalid-argument', 'Máximo de 300 registros por execução.');
    }
    const db = admin.firestore();
    ...
    snaps.forEach((snap) => {
        ...
        if (!snap.exists || !data || data.organization_id !== organizationId)
            return;
        if (!isEligible(data, cutoff))
            return;
