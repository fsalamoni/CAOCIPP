...
exports.concludeAditivo = void 0;
...
exports.concludeAditivo = (0, https_1.onCall)({ region: 'southamerica-east1' }, async (request) => {
    ...
    const result = await db.runTransaction(async (t) => {
        ...
        if (aditivo.status === 'Concluído') {
            throw new https_1.HttpsError('failed-precondition', 'Este aditivo já está concluído.');
        }
        ...
        if (!scopeUndefined) {
            if (allowProrrogacao && !hasPrazo) {
                throw new https_1.HttpsError('failed-precondition', 'Este aditivo é de prorrogação: informe o prazo (quantidade + unidade).');
            }
            if (allowObjeto && !hasObjeto) {
                throw new https_1.HttpsError('failed-precondition', 'Este aditivo é de objeto: informe o objeto do aditivo.');
            }
        }
