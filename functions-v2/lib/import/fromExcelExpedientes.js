...
exports.importExpedientesFromExcel = void 0;
...
exports.importExpedientesFromExcel = (0, https_1.onCall)({
    ...
    try {
        ...
        if (rows.length > 50000) {
            throw new https_1.HttpsError('invalid-argument', `Arquivo muito grande (${rows.length} expedientes). Máximo: 50.000.`);
        }
        ...
        for (let i = 0; i < rows.length; i += lookupBatchSize) {
            ...
            const lookupPromises = chunk.map(async (row, index) => {
                ...
                try {
                    ...
                    // Extract entry date
                    const entryDate = parseExcelDate(row['ENTRADA'] || row['Entrada'] || row['entrada'] ||
                ...
                try {
                    ...
                    const expedienteData = {
                        ...
                        urgency_request: (row['PEDIDO DE URGÊNCIA'] === 'Sim' ||
                            ...
                            row['urgency_request'] === 'true'),
                        observations: (row['OBSERVAÇÕES'] || row['Observações'] || row['Obs'] || row['observations'] || '').toString().trim(),
                        distribution_date: parseExcelDate(row['DISTRIBUIÇÃO'] || row['Distribuição'] || row['distribuição'] || row['distribution_date']) || null,
                        analysis_start_date: parseExcelDate(row['INÍCIO DA ANÁLISE'] || row['Início da Análise'] ||
                            row['INÍCIO ANÁLISE'] || row['analysis_start_date']) || null,
                        review_submission_date: parseExcelDate(row['REMESSA P/ REVISÃO'] || row['Remessa p/ Revisão'] ||
                            row['REMESSA PARA REVISÃO'] || row['Remessa'] || row['review_submission_date']) || null,
                        review_return_date: parseExcelDate(row['DEVOLUÇÃO APÓS REVISÃO'] || row['Devolução após Revisão'] ||
                            row['DEVOLUÇÃO'] || row['Devolução'] || row['review_return_date']) || null,
                        archived_date: parseExcelDate(row['ARQUIVAMENTO'] || row['Arquivamento'] || row['arquivamento'] || row['archived_date']) || null,
                    ...
                    expedienteData.status = (0, status_1.calculateStatus)(expedienteData);
                    // Build log entry
                    const fieldLabels = {
                    ...
                    if (isUpdate) {
                        ...
                        for (const [key, label] of Object.entries(fieldLabels)) {
                            ...
                            const newVal = ((_b = expedienteData[key]) !== null && _b !== void 0 ? _b : '').toString().trim();
                    ...
                    if (isUpdate) {
                        Object.keys(expedienteData).forEach(key => {
                            if (expedienteData[key] === undefined)
                                delete expedienteData[key];
                        });
                    ...
                    batch.set(expedienteRef.collection('history').doc((0, history_1.historyEntryId)(importLogEntry)), Object.assign(Object.assign({}, importLogEntry), { created_at: admin.firestore.FieldValue.serverTimestamp() }));
                    batchCount += 2;
        ...
        if ((error === null || error === void 0 ? void 0 : error.code) === 'functions/unauthenticated' || (error === null || error === void 0 ? void 0 : error.code) === 'functions/permission-denied' || (error === null || error === void 0 ? void 0 : error.code) === 'functions/invalid-argument') {
            throw error;
        }
...
//# sourceMappingURL=fromExcelExpedientes.js.map