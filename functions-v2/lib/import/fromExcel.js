        ...
        for (let i = 0; i < rows.length; i += lookupBatchSize) {
            ...
            const lookupPromises = chunk.map(async (row, index) => {
                ...
                try {
                    ...
                    const locationVal = (row['LOCAL DOS FATOS\n(CIDADE)'] ||
                        row['LOCAL DOS FATOS\\n(CIDADE)'] ||
                        row['Local'] || row['LOCAL'] || row['Município'] || row['location'] || '').toString().trim();
                    const objectVal = (row['MATÉRIA E OBJETO DA CONSULTA'] ||
                        row['Objeto'] || row['OBJETO'] || row['Assunto'] || row['matter_object'] || '').toString().trim();
                    // Extract entry date
                    const entryDate = parseExcelDate(row['ENTRADA NO CAOPP\n(DATA)'] ||
                        row['ENTRADA NO CAOPP\\n(DATA)'] ||
                        row['Data Entrada'] || row['DATA ENTRADA'] || row['entry_date']) || null;
            ...
            for (const res of results) {
                ...
                try {
                    ...
                    const processData = {
                        ...
                        responsible_user_name: (row['ASSESSOR RESPONSÁVEL'] || row['Responsável'] || row['RESPONSÁVEL'] || row['responsible_user_name'] || '').toString().trim() ? (0, normalization_1.formatPersonName)((row['ASSESSOR RESPONSÁVEL'] || row['Responsável'] || row['RESPONSÁVEL'] || row['responsible_user_name'] || '').toString().trim()) : null,
                        ...
                        observations: (row['OBSERVAÇÕES E PONTOS IMPORTANTES DA RESPOSTA'] ||
                            row['Observações'] || row['Obs'] || row['observations'] || '').toString().trim(),
