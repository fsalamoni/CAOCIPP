...
exports.importParceriasFromExcel = void 0;
...
function normalizePartnershipType(raw) {
    if (!raw)
        return '';
    const norm = String(raw).toLowerCase().trim();
    return PARTNERSHIP_TYPE_MAP[norm] || '';
}
exports.importParceriasFromExcel = (0, https_1.onCall)({
    ...
    try {
        ...
        if (rows.length > 50000) {
            throw new https_1.HttpsError('invalid-argument', `Arquivo muito grande (${rows.length} parcerias). Máximo: 50.000.`);
        }
        ...
        for (let i = 0; i < rows.length; i += lookupBatchSize) {
            ...
            const lookupPromises = chunk.map(async (row, index) => {
                ...
                try {
                    ...
                    const signatureDate = parseExcelDate(pickFirst(row, [
                    ...
                    const endDate = parseExcelDate(pickFirst(row, [
                        'TERMO FINAL', 'Termo Final', 'termo_final', 'end_date',
                    ]));
                    const renewalNoticeDate = parseExcelDate(pickFirst(row, [
            ...
            for (const res of results) {
                ...
                try {
                    ...
                    parceriaData.status = (0, status_1.calculateParceriaStatus)(parceriaData);
                    const fieldLabels = {
                        pgea: 'PGEA',
                        subject: 'Assunto',
                        object: 'Objeto',
                    ...
                    if (isUpdate) {
                        ...
                        for (const [key, label] of Object.entries(fieldLabels)) {
                            ...
                            const newVal = ((_b = parceriaData[key]) !== null && _b !== void 0 ? _b : '').toString().trim();
                    ...
                    if (isUpdate) {
                        Object.keys(parceriaData).forEach((key) => {
                            if (parceriaData[key] === undefined)
                                delete parceriaData[key];
                        });
                    ...
                    batch.set(parceriaRef.collection('history').doc((0, history_1.historyEntryId)(importLogEntry)), Object.assign(Object.assign({}, importLogEntry), { created_at: admin.firestore.FieldValue.serverTimestamp() }));
                    batchCount += 2;
        ...
        if ((error === null || error === void 0 ? void 0 : error.code) === 'functions/unauthenticated' ||
            (error === null || error === void 0 ? void 0 : error.code) === 'functions/permission-denied' ||
            (error === null || error === void 0 ? void 0 : error.code) === 'functions/invalid-argument') {
            throw error;
        }
...
//# sourceMappingURL=fromExcelParcerias.js.map