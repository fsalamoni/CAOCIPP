...
export const sendLoginOtp = onCall<void>(
    ...
    async (request) => {
        ...
        const lastSentAt = existing.data()?.created_at?.toMillis?.();
        if (lastSentAt && Date.now() - lastSentAt < RESEND_COOLDOWN_MS) {
            throw new HttpsError('resource-exhausted', 'Aguarde alguns segundos antes de solicitar um novo código.');
        }
...
export const verifyLoginOtp = onCall<{ code: string }>(
    { region: 'southamerica-east1' },
    async (request) => {
        if (!request.auth) {
            throw new HttpsError('unauthenticated', 'Authenticated user required');
        ...
        if (!snap.exists) {
            throw new HttpsError('failed-precondition', 'Nenhum código pendente. Solicite um novo.');
        }
        const data = snap.data();
        if (!data || data.expires_at.toMillis() < Date.now()) {
            await ref.delete();
            throw new HttpsError('failed-precondition', 'Código expirado. Solicite um novo.');
        }
