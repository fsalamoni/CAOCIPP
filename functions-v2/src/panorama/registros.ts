 ...
 */
function chaveDe(values: Record<string, unknown>, def: PanoramaBaseDef): string {
    const col = columnForRole(def, 'identificador');
    if (!col) return '';
    return normalizeIdentifier(values[col.key]);
}

export const managePanoramaRegistro = onCall<RegistroRequest>(
    ...
    async (request) => {
        ...
        const payload = request.data || ({} as RegistroRequest);
        ...
        if (!baseId) throw new HttpsError('invalid-argument', 'baseId é obrigatório');
        ...
        if (action === 'delete') {
            if (!membership.canDelete) {
                throw new HttpsError(
                    'permission-denied',
                    'É preciso a permissão "Excluir registros" para excluir'
                );
            }
            const alvos = payload.ids?.length
                ? payload.ids
                : (payload.id ? [payload.id] : []);
            if (alvos.length === 0) throw new HttpsError('invalid-argument', 'Informe o registro a excluir');
            if (alvos.length > MAX_BULK) {
                throw new HttpsError('invalid-argument', `Máximo de ${MAX_BULK} registros por vez`);
            }
            ...
            for (let i = 0; i < alvos.length; i += BATCH_SIZE) {
                ...
                snaps.forEach((snap) => {
                    ...
                    if (!snap.exists || data?.organization_id !== organizationId
                        || data?.base_id !== baseId) {
                        ignorados += 1;
                        return;
                    }
        ...
        if (action === 'bulkUpdate') {
            ...
            if (alvos.length === 0) throw new HttpsError('invalid-argument', 'Nenhum registro selecionado');
            if (alvos.length > MAX_BULK) {
                throw new HttpsError('invalid-argument', `Máximo de ${MAX_BULK} registros por vez`);
            }

            const patch = sanitizeRecordValues(payload.values, def);
            ...
            const colId = columnForRole(def, 'identificador');
            if (colId && chaves.includes(colId.key)) {
                throw new HttpsError(
                    'invalid-argument',
                    'O identificador não pode ser alterado em massa: ele é a chave de cada registro'
                );
            }

            const rotulos = chaves.map(
                (k) => def.columns.find((c) => c.key === k)?.label || k
            );
        ...
        const values = sanitizeRecordValues(payload.values, def);
        ...
        if (action === 'update') {
            ...
            const merged = { ...(atual.values || {}), ...values };
            ...
            if (novaChave && novaChave !== atual.chave_norm) {
                ...
                if (!conflito.empty && conflito.docs[0].id !== id) {
                    throw new HttpsError(
                        'already-exists',
                        'Já existe um registro nesta base com esse identificador'
                    );
                }
            }

            const alterados = Object.keys(values).filter(
                (k) => String(atual.values?.[k] ?? '') !== String(values[k] ?? '')
            );
            if (alterados.length === 0) {
                return { success: true, id, semAlteracao: true };
            }
            const rotulos = alterados.map(
                (k) => def.columns.find((c) => c.key === k)?.label || k
            );
            ...
            await snap.ref.update({
                ...
                activity_log: admin.firestore.FieldValue.arrayUnion(
                    logEntry(`Campos atualizados: ${rotulos.join(', ')}`)
                ),
            });
        ...
        if (colId && !chave) {
            throw new HttpsError(
                'invalid-argument',
                `O campo "${colId.label}" é o identificador desta base e é obrigatório`
            );
        }
