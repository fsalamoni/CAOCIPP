...
export interface PanoramaRoleDef {
    key: PanoramaRole;
    label: string;
    /** O que este papel destrava quando preenchido. */
    destrava: string;
    /** Tipos de coluna aceitáveis para o papel. */
    tipos: PanoramaColumnType[];
    /** Um papel só pode ser ocupado por UMA coluna? */
    unico: boolean;
}

export const PANORAMA_ROLES: PanoramaRoleDef[] = [
    {
        key: 'identificador',
...
export const PANORAMA_ROLE_KEYS: PanoramaRole[] = PANORAMA_ROLES.map((r) => r.key);

// ----------------------------------------------------------------------------
// Tipos de coluna
// ----------------------------------------------------------------------------

export type PanoramaColumnType = 'texto' | 'numero' | 'moeda' | 'data' | 'booleano' | 'lista';

export const PANORAMA_COLUMN_TYPES: PanoramaColumnType[] = [
    'texto', 'numero', 'moeda', 'data', 'booleano', 'lista',
];

export interface PanoramaColumn {
    /** Chave estável, derivada do cabeçalho. Nunca muda depois de criada. */
    key: string;
    /** Rótulo exibido. O admin pode renomear sem quebrar os dados. */
    label: string;
    type: PanoramaColumnType;
    /** Papel semântico, ou null quando a coluna é apenas informativa. */
    role: PanoramaRole | null;
    /** Valores canônicos, para colunas do tipo lista. */
    lista?: string[];
    /** Quantos valores distintos foram vistos na última importação. */
    cardinalidade?: number;
    /** Amostra de valores, para o admin reconhecer a coluna na configuração. */
    amostra?: string[];
    /** Quantas linhas tinham esta coluna vazia, na última importação. */
    vazios?: number;
    visivel: boolean;
    ordem: number;
    /** Cabeçalho original da planilha, para o log da importação. */
    origem?: string;
}
...
export function slugColumnKey(label: string, taken: Set<string> = new Set()): string {
 ...
 */
export function parseDateValue(raw: unknown): string {
...
export function parseBooleanValue(raw: unknown): boolean | null {
    ...
    if (FALSOS.has(s)) return s !== '' ? false : null;
    return null;
}
...
export function normalizeIdentifier(raw: unknown): string {
    ...
    if (!/[a-z]/.test(normalizado)) return normalizado.replace(/\D/g, '');
...
export function coerceValue(raw: unknown, type: PanoramaColumnType): string | number | boolean | null {
...
export interface ColumnStats {
    ...
    distintos: number;
    amostra: string[];
    /** Quantos valores não vazios foram reconhecidos em cada tipo. */
    datas: number;
    numeros: number;
    moedas: number;
    booleanos: number;
    /** Valores distintos, quando poucos o bastante para virarem lista. */
    valores: string[];
}

/** Estatísticas de uma coluna a partir dos valores brutos daquela coluna. */
export function analyzeColumn(valores: unknown[]): ColumnStats {
    ...
    for (const raw of valores) {
        const s = String(raw ?? '').trim();
        if (!s) { vazios += 1; continue; }

        distintos.set(s, (distintos.get(s) || 0) + 1);

        if (parseDateValue(raw)) datas += 1;
        const n = parseNumberValue(raw);
        if (n !== null) {
            numeros += 1;
            // "R$" ou duas casas decimais com separador de milhar denunciam dinheiro.
            if (/R\$/i.test(s) || /\d{1,3}(\.\d{3})+,\d{2}$/.test(s) || /,\d{2}$/.test(s)) moedas += 1;
        }
        if (parseBooleanValue(raw) !== null && s.length <= 12) booleanos += 1;
    }
 ...
 */
export function inferColumnType(stats: ColumnStats): PanoramaColumnType {
    const preenchidos = stats.total - stats.vazios;
    if (preenchidos === 0) return 'texto';
    const proporcao = (n: number) => n / preenchidos;

    if (proporcao(stats.booleanos) >= 0.9 && stats.distintos <= 3) return 'booleano';
    if (proporcao(stats.datas) >= 0.8) return 'data';
    if (proporcao(stats.numeros) >= 0.8) {
        return proporcao(stats.moedas) >= 0.5 ? 'moeda' : 'numero';
    }
    if (stats.distintos <= LISTA_MAX_DISTINTOS
        && proporcao(stats.distintos) <= LISTA_MAX_PROPORCAO) return 'lista';
    return 'texto';
}
 ...
 */
const PISTAS: Record<PanoramaRole, string[]> = {
    identificador: [
...
export interface RoleSuggestion {
    role: PanoramaRole;
    /** 0 a 1 — quanto a plataforma confia na sugestão. */
    confianca: number;
    /** Por que sugeriu, em português, para aparecer na tela de mapeamento. */
    motivo: string;
}
...
export function suggestRole(
    ...
    let melhor: RoleSuggestion | null = null;
    ...
    const propor = (role: PanoramaRole, confianca: number, motivo: string, pista = 0) => {
        if (!melhor
    ...
    for (const role of PANORAMA_ROLE_KEYS) {
        const def = PANORAMA_ROLES.find((r) => r.key === role)!;
        if (!def.tipos.includes(type)) continue;
        ...
        propor(
            role, 0.85,
            `o nome da coluna contém "${pista}" e o conteúdo é do tipo ${type}`,
            pista.length
        );
    }
    ...
    if (type === 'texto' && proporcaoDistintos > 0.95 && preenchidos >= 5) {
        ...
        if (pareceIdentificador) {
            propor(
                'identificador', melhor ? 0.9 : 0.7,
                `${Math.round(proporcaoDistintos * 100)}% dos valores são distintos e têm forma de código`,
                99
            );
        }
    }
    ...
    if ((type === 'lista' || type === 'texto')
        ...
        propor(
            'unidade', 0.35,
            `${stats.distintos} valores distintos em ${preenchidos} linhas — parece uma dimensão de agrupamento`
        );
    }
...
export interface PanoramaDesfechoConfig {
    /** Peso de 0 a 1 de cada desfecho, para o aproveitamento ponderado. */
    pesos: Record<string, number>;
    /** Desfechos que NÃO representam solução de mérito (o análogo da dissolução). */
    neutros: string[];
    /** Cor de fundo da etiqueta de cada desfecho. */
    cores: Record<string, string>;
}

export interface PanoramaPrescricao {
    /** 'coluna' usa a data-limite já calculada; 'prazo' conta a partir de uma data. */
    modo: 'desligado' | 'coluna' | 'prazo';
    /** Anos a contar, quando o modo é 'prazo'. */
    anosPadrao: number;
    /** Prazo específico por valor de assunto ("Improbidade": 5). */
    anosPorAssunto: Record<string, number>;
    /** De qual data conta: a principal ou a de referência. */
    contarDe: 'data_principal' | 'data_referencia';
    /** Faixas de alerta, em dias restantes (da mais urgente para a menos). */
    alertas: number[];
}

export interface PanoramaBaseDef {
    nome: string;
    descricao: string;
    columns: PanoramaColumn[];
    desfechos: PanoramaDesfechoConfig;
    /** Agrupamentos de unidades: { "Região Metropolitana": ["Canoas", ...] }. */
    regioes: Record<string, string[]>;
    prescricao: PanoramaPrescricao;
    importPolicy: 'preserve' | 'update';
    fuzzyThreshold: number;
}

export const PANORAMA_PRESCRICAO_PADRAO: PanoramaPrescricao = {
    modo: 'desligado',
    anosPadrao: 5,
    anosPorAssunto: {},
...
export const PANORAMA_DEFAULT_DEF: PanoramaBaseDef = {
    nome: 'Nova base',
    descricao: '',
    columns: [],
...
function sanitizeColumns(input: unknown): PanoramaColumn[] {
    const arr = Array.isArray(input) ? input : [];
    const out: PanoramaColumn[] = [];
    ...
    const papeisUsados = new Set<string>();
    ...
    for (const raw of arr) {
        ...
        const keyRecebida = String(item.key ?? '').trim().slice(0, 60);
        ...
        const type = PANORAMA_COLUMN_TYPES.includes(item.type as PanoramaColumnType)
            ? item.type as PanoramaColumnType
            : 'texto';

        let role: PanoramaRole | null = null;
        const roleRaw = String(item.role ?? '');
        if (PANORAMA_ROLE_KEYS.includes(roleRaw as PanoramaRole)) {
            const def = PANORAMA_ROLES.find((r) => r.key === roleRaw)!;
        ...
        out.push({
            ...
            lista: Array.isArray(item.lista)
                ? [...new Set(item.lista.map((v) => String(v ?? '').trim()).filter(Boolean))].slice(0, 2000)
                : undefined,
            ...
            amostra: Array.isArray(item.amostra)
                ? item.amostra.map((v) => String(v ?? '').slice(0, 80)).slice(0, 8)
                : undefined,
...
function sanitizeDesfechos(input: unknown): PanoramaDesfechoConfig {
    const cfg = (input && typeof input === 'object' ? input : {}) as Record<string, unknown>;

    const pesos: Record<string, number> = {};
    const rawPesos = (cfg.pesos && typeof cfg.pesos === 'object' ? cfg.pesos : {}) as Record<string, unknown>;
    for (const [chave, valor] of Object.entries(rawPesos)) {
        if (Object.keys(pesos).length >= MAX_DESFECHOS_CONFIGURAVEIS) break;
    ...
    const rawCores = (cfg.cores && typeof cfg.cores === 'object' ? cfg.cores : {}) as Record<string, unknown>;
    for (const [chave, valor] of Object.entries(rawCores)) {
        if (Object.keys(cores).length >= MAX_DESFECHOS_CONFIGURAVEIS) break;
        const nome = chave.trim().slice(0, 160);
        if (!nome) continue;
        const cor = sanitizeHexColor(valor);
        if (cor) cores[nome] = cor;
    }
...
function sanitizeRegioes(input: unknown): Record<string, string[]> {
    const raw = (input && typeof input === 'object' ? input : {}) as Record<string, unknown>;
    const out: Record<string, string[]> = {};
    let n = 0;
    for (const [nome, membros] of Object.entries(raw)) {
        const chave = String(nome ?? '').trim().slice(0, 120);
        if (!chave || !Array.isArray(membros)) continue;
        const lista = [...new Set(membros.map((m) => String(m ?? '').trim()).filter(Boolean))].slice(0, 600);
...
function sanitizePrescricao(input: unknown): PanoramaPrescricao {
    const base = PANORAMA_PRESCRICAO_PADRAO;
    const cfg = (input && typeof input === 'object' ? input : {}) as Record<string, unknown>;
    const modo = ['desligado', 'coluna', 'prazo'].includes(String(cfg.modo))
        ? cfg.modo as PanoramaPrescricao['modo']
        : base.modo;

    const anosPorAssunto: Record<string, number> = {};
    const raw = (cfg.anosPorAssunto && typeof cfg.anosPorAssunto === 'object'
        ? cfg.anosPorAssunto : {}) as Record<string, unknown>;
...
export function sanitizeBaseDef(input: unknown): PanoramaBaseDef {
    const cfg = (input && typeof input === 'object' ? input : {}) as Record<string, unknown>;
    const columns = sanitizeColumns(cfg.columns);
    const fuzzyRaw = Number(cfg.fuzzyThreshold);

    return {
        nome: String(cfg.nome ?? '').trim().slice(0, 120) || 'Nova base',
        descricao: String(cfg.descricao ?? '').trim().slice(0, 600),
        columns,
...
export function columnForRole(def: PanoramaBaseDef, role: PanoramaRole): PanoramaColumn | null {
...
export function sanitizeRecordValues(
    ...
    for (const col of def.columns) {
        if (!Object.prototype.hasOwnProperty.call(raw, col.key)) continue;
 ...
 */
export function fuzzyMatchFromList(raw: unknown, lista: string[], threshold: number): FuzzyMatch {
